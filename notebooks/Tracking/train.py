...
with mlflow.start_run(run_name="PARENT_RUN") as parent_run:
    ...
    with mlflow.start_run(
        run_name="MATRIX_EVALUATION", nested=True
    ) as child_run:
    ...
    with mlflow.start_run(run_name="CLEANING_DATA", nested=True) as child_run:
        ...
        )  # Dropped Ocean_Proximity as it is a non-numeric column.
        ...
        housing_cat_1hot = cat_encoder.fit_transform(
            housing_cat
        )  # Creating Dummy clomns for non-numeric data

    with mlflow.start_run(
        run_name="CUSTOM_TRANSFORMER", nested=True
    ) as child_run:
        mlflow.log_param("child", "yes")
    ...
    with mlflow.start_run(run_name="PIPELINE", nested=True) as child_run:
        ...
        num_pipeline = Pipeline(
            [
                ...
                ),  ## combining attributes to make them logical
    ...
    with mlflow.start_run(run_name="TRAINING_MODEL", nested=True) as child_run:
        ...
        with mlflow.start_run(
            run_name="RANDOM_FOREST_MODEL", nested=True
        ) as child_run:
            mlflow.log_param("child", "yes")
        ...
        with mlflow.start_run(
            run_name="LINEAR_REGRESSION_MODEL", nested=True
        ) as child_run:
            mlflow.log_param("child", "yes")
